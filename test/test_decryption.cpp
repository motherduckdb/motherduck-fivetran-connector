...
void encrypt_stream(std::istream &input, std::ostream &output, const std::string &key) {
	...
	if (key.size() != static_cast<size_t>(aes_key_length)) {
		...
		throw std::runtime_error("Keys for algorithm " + std::string(algorithm) + " must be " +
		                         std::to_string(aes_key_length) + " bytes long, but the provided key has length " +
	...
	EVP_CIPHER_free(aes_impl);
	if (init_result != 1) {
		openssl_helper::raise_openssl_error("Failed to initialize encryption cipher context for " +
		                                    std::string(algorithm));
	}
