...
TEST_CASE("Migrate - fails with empty table name", "[integration][migrate]") {
	...
	CHECK_THAT(status.error_message(), Catch::Matchers::ContainsSubstring("Table name cannot be empty"));
}
...
TEST_CASE("Migrate - works with schema", "[integration][migrate]") {
	...
	{
		auto res =
		    con->Query("CREATE TABLE " + schema_name + "." + table_name + " (id INT PRIMARY KEY, value VARCHAR)");
		REQUIRE_NO_FAIL(res);
	}
	{
		auto res = con->Query("INSERT INTO " + schema_name + "." + table_name + " VALUES (1, 'test')");
		REQUIRE_NO_FAIL(res);
	}
	...
	{
		auto res = con->Query("SELECT new_value FROM " + schema_name + "." + table_name + " WHERE id = 1");
		REQUIRE_NO_FAIL(res);
		REQUIRE(res->RowCount() == 1);
