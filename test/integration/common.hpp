...
inline bool REQUIRE_FAIL(const grpc::Status& status, const std::string& expected_error) {
	if (!status.ok()) {
...
inline bool REQUIRE_FAIL(const grpc::Status& status, const Catch::Matchers::StringMatcherBase& matcher) {
	if (!status.ok()) {
...
template <typename T>
void add_col(T& request, const std::string& name, ::fivetran_sdk::v2::DataType type, bool is_primary_key) {
...
template <typename T>
void define_history_test_table_reordered(T& request, const std::string& table_name) {
	request.mutable_table()->set_name(table_name);
	add_col(request, "_fivetran_end", ::fivetran_sdk::v2::DataType::UTC_DATETIME, false);
