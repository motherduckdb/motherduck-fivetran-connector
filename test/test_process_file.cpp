...
TEST_CASE("Test can read simple CSV file", "[csv_processor]") {
	...
	std::vector<column_def> columns {column_def {.name = "id", .type = duckdb::LogicalType::INTEGER},
...
TEST_CASE("Test reading CSV file with auto-detection of column types", "[csv_processor]") {
	...
	std::vector<column_def> columns {column_def {.name = "id", .type = duckdb::LogicalType::INVALID},
...
TEST_CASE("Test reading CSV file with columns out of order", "[csv_processor]") {
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "csv" / "small_simple.csv";
	CAPTURE(test_file);
	REQUIRE(fs::exists(test_file));
...
TEST_CASE("Test reading CSV file with quotes in filename", "[csv_processor]") {
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "csv" / "filename_with_'quotes'.csv";
	CAPTURE(test_file);
	REQUIRE(fs::exists(test_file));
	...
	auto [filename, row_count, columns] = GENERATE(table<std::string, size_t, std::vector<column_def>>(
	    {std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         ...
	         {{.name = "id", .type = duckdb::LogicalTypeId::INTEGER},
	          {.name = "bool_true_false", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         "dates_and_times.csv", 5,
	         {{.name = "date", .type = duckdb::LogicalTypeId::DATE},
	          {.name = "time", .type = duckdb::LogicalTypeId::TIME}, // TODO: Do we want this column?
	                                                                 // Fivetran doesn't support TIME
	                                                                 // type
	          {.name = "datetime", .type = duckdb::LogicalTypeId::TIMESTAMP},
	          {.name = "timestamp", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ}}),
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         "large.csv", 1000,
	     ...
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         "many_columns.csv", 3, {{.name = "col1", .type = duckdb::LogicalTypeId::VARCHAR},
	     ...
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         ...
	         {{.name = "smallint_col", .type = duckdb::LogicalTypeId::SMALLINT},
	          ...
	          {.name = "decimal_5_3_col", .type = duckdb::LogicalTypeId::DECIMAL, .width = 5, .scale = 3}}),
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         "single_column.csv", 10, {{.name = "value", .type = duckdb::LogicalTypeId::SMALLINT}}),
	     std::make_tuple<std::string, size_t, std::vector<column_def>>(
	         "special_chars.csv", 6,
	         {{.name = "id", .type = duckdb::LogicalTypeId::INTEGER},
	          {.name = "text", .type = duckdb::LogicalTypeId::VARCHAR},
	          {.name = "description", .type = duckdb::LogicalTypeId::VARCHAR}})}));
...
TEST_CASE("Test reading CSV file with special null string", "[csv_processor]") {
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "csv" / "special_null.csv";
	REQUIRE(fs::exists(test_file));
...
TEST_CASE("Test reading CSV file with escaped string", "[csv_processor]") {
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "csv" / "escaped_string.csv";
	REQUIRE(fs::exists(test_file));
...
TEST_CASE("Test reading CSV file with unmodified string setting", "[csv_processor]") {
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "csv" / "unmodified_string.csv";
	REQUIRE(fs::exists(test_file));
...
TEST_CASE("Test reading CSV file with BINARY column", "[csv_processor]") {
	const auto [base64_data, decoded_string] = GENERATE(table<std::string, std::string>(
	    {std::make_tuple<std::string, std::string>("3q2+7w==", R"(\xDE\xAD\xBE\xEF)"),
	     std::make_tuple<std::string, std::string>("AAECAwQFBgc=", R"(\x00\x01\x02\x03\x04\x05\x06\x07)"),
	     std::make_tuple<std::string, std::string>("SGVsbG8gV29ybGQh", "Hello World!"),
	...
	duckdb::DuckDB db(nullptr);
	duckdb::Connection con(db);
	const std::vector<column_def> columns {column_def {.name = "binary_val", .type = duckdb::LogicalType::BLOB}};
...
TEST_CASE("Test reading zstd-compressed CSV files", "[csv_processor]") {
	...
	const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "compressed_csv" / filename;
	CAPTURE(test_file);
	REQUIRE(fs::exists(test_file));
...
TEST_CASE("Test reading files generated by Fivetran destination tester", "[csv_processor]") {
	...
	auto [filename, row_count, null_string, can_contain_unmodified, columns] =
	    GENERATE(table<std::string, size_t, std::string, bool, std::vector<column_def>>({
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "campaign_input_1_upsert", 3, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "num", .type = duckdb::LogicalTypeId::DECIMAL, .width = 6, .scale = 3},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "_fivetran_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	             {.name = "_fivetran_deleted", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "campaign_input_1_update", 1, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", true,
	            {{.name = "name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "num", .type = duckdb::LogicalTypeId::DECIMAL, .width = 6, .scale = 3},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "_fivetran_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	             {.name = "_fivetran_deleted", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "campaign_input_1_delete", 1, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "num", .type = duckdb::LogicalTypeId::DECIMAL, .width = 6, .scale = 3},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "_fivetran_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "transaction_input_1_upsert", 7, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "id", .type = duckdb::LogicalTypeId::INTEGER, .primary_key = true},
	             {.name = "amount", .type = duckdb::LogicalTypeId::FLOAT},
	             {.name = "desc", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "_fivetran_deleted", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "transaction_input_1_update", 4, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", true,
	            {{.name = "id", .type = duckdb::LogicalTypeId::INTEGER, .primary_key = true},
	             {.name = "amount", .type = duckdb::LogicalTypeId::FLOAT},
	             {.name = "desc", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "_fivetran_deleted", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "transaction_input_1_delete", 2, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "id", .type = duckdb::LogicalTypeId::INTEGER, .primary_key = true},
	             {.name = "amount", .type = duckdb::LogicalTypeId::FLOAT},
	             {.name = "desc", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "orders_input_2_upsert", 3, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "order_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	             ...
	             {.name = "_fivetran_deleted", .type = duckdb::LogicalTypeId::BOOLEAN}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "orders_input_2_update", 3, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", true,
	            {{.name = "order_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	             {.name = "customer_id", .type = duckdb::LogicalTypeId::BIGINT},
	             {.name = "order_date", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "status", .type = duckdb::LogicalTypeId::VARCHAR},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "customers_input_2_upsert", 6, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "customer_id", .type = duckdb::LogicalTypeId::BIGINT, .primary_key = true},
	             ...
	             {.name = "total_spent", .type = duckdb::LogicalTypeId::DECIMAL, .width = 12, .scale = 2},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             {.name = "loyalty_tier", .type = duckdb::LogicalTypeId::VARCHAR}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "customers_input_2_update", 2, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", true,
	            {{.name = "customer_id", .type = duckdb::LogicalTypeId::BIGINT, .primary_key = true},
	             {.name = "first_name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "last_name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "email", .type = duckdb::LogicalTypeId::VARCHAR},
	             ...
	             {.name = "loyalty_tier", .type = duckdb::LogicalTypeId::VARCHAR}}),
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "customers_input_2_delete", 1, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "customer_id", .type = duckdb::LogicalTypeId::BIGINT, .primary_key = true},
	             {.name = "first_name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "last_name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "email", .type = duckdb::LogicalTypeId::VARCHAR},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "products_input_2_upsert", 5, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "product_name", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "category", .type = duckdb::LogicalTypeId::VARCHAR},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "user_profiles_input_3_upsert", 24, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "user_id", .type = duckdb::LogicalTypeId::BIGINT, .primary_key = true},
	        ...
	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "web_events_input_3_upsert", 154, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "event_id", .type = duckdb::LogicalTypeId::VARCHAR, .primary_key = true},
	             {.name = "session_id", .type = duckdb::LogicalTypeId::VARCHAR},
	             {.name = "user_id", .type = duckdb::LogicalTypeId::BIGINT},
	             {.name = "event_timestamp", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	             ...
	             {.name = "processing_time_ms", .type = duckdb::LogicalTypeId::INTEGER},
	             {.name = "is_error", .type = duckdb::LogicalTypeId::BOOLEAN},
	             {.name = "_fivetran_synced", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ}}),

	        std::make_tuple<std::string, size_t, std::string, bool, std::vector<column_def>>(
	            "mixed_data_input_4_upsert", 1, "null-m8yilkvPsNulehxl2G6pmSQ3G3WWdLP", false,
	            {{.name = "bool_val", .type = duckdb::LogicalTypeId::BOOLEAN},
	             ...
	             {.name = "decimal_val", .type = duckdb::LogicalTypeId::DECIMAL, .width = 38, .scale = 10},
	             {.name = "naive_time_val", .type = duckdb::LogicalTypeId::TIME},
	             {.name = "naive_date_val", .type = duckdb::LogicalTypeId::DATE},
	             {.name = "naive_datetime_val", .type = duckdb::LogicalTypeId::TIMESTAMP},
	             {.name = "utc_datetime_val", .type = duckdb::LogicalTypeId::TIMESTAMP_TZ},
	...
	const fs::path test_file =
	    fs::path(TEST_RESOURCES_DIR) / "destination_tester" / "generated_files" / (filename + file_extension);
	CAPTURE(test_file);
	REQUIRE(fs::exists(test_file));
	...
	if (duckdb::StringUtil::EndsWith(file_extension, ".aes")) {
		...
		decryption_key.assign(std::istreambuf_iterator<char>(key_stream), std::istreambuf_iterator<char>());
	}
...
TEST_CASE("Test reading a CSV file with a huge VARCHAR column", "[csv_processor]") {
	SECTION("Fails to read a CSV file with a 27 MB VARCHAR column throws the right RecoverableError") {
		...
		IngestProperties props {.filename = test_file.string(), .columns = columns};
		auto logger = mdlog::Logger::CreateNopLogger();
		REQUIRE_THROWS_AS(csv_processor::ProcessFile(con, props, logger,
		                                             [](const std::string&) {
			                                             // Do nothing
		                                             }),
	...
	SECTION("Reading a CSV file with a 27 MB VARCHAR column succeeds with increased max_line_size") {
		...
		csv_processor::ProcessFile(con, props, logger, [&con](const std::string& staging_table_name) {
			const auto res = con.Query("FROM " + staging_table_name);
			if (res->HasError()) {
				FAIL("Failed to query staging table: " + res->GetError());
			}
			REQUIRE_FALSE(res->HasError());
			REQUIRE(res->RowCount() == 1);
		});
	...
	SECTION("Reading a CSV file with a 15 MB VARCHAR column succeeds") {
		const fs::path test_file = fs::path(TEST_RESOURCES_DIR) / "compressed_csv" / "lorem_ipsum_20mb.csv.zst";
		REQUIRE(fs::exists(test_file));
