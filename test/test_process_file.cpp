...
TEST_CASE("Test reading CSV file with columns out of order",
  ...
  std::vector<column_def> columns{
      column_def{.name = "name", .type = duckdb::LogicalType::VARCHAR},
      column_def{.name = "age",
                 .type = duckdb::LogicalType::SMALLINT},
      column_def{
...
TEST_CASE("Test reading files generated by Fivetran destination tester",
    ...
    auto [filename, row_count, null_string, can_contain_unmodified, columns] = GENERATE(table<std::string, size_t, std::string, bool, std::vector<column_def>>(
  ...
  duckdb::Connection con(db);
    ...
    const std::string decryption_key = duckdb::StringUtil::EndsWith(file_extension, ".aes")
        ? test_file.string() + ".key" : "";

  IngestProperties props(test_file.string(), decryption_key, columns, null_string, 1);
    props.allow_unmodified_string = can_contain_unmodified;

  auto logger = std::make_shared<mdlog::MdLog>();
    csv_processor::ProcessFile(con, props, logger, [&con, expected_row_count = row_count](const std::string &view_name) {
          const auto res = con.Query("FROM " + view_name);
          REQUIRE_FALSE(res->HasError());
          REQUIRE(res->RowCount() == expected_row_count);
      });
