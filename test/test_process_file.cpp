...
TEST_CASE("Test reading various CSV files", "[csv_processor]") {
    auto [filename, row_count, columns] = GENERATE(table<std::string, size_t, std::vector<column_def>>({
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("dates_and_times.csv", 5, {
            ...
            { .name = "time", .type = duckdb::LogicalTypeId::TIME }, // TODO: Do we want this column? Fivetran doesn't support TIME type
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("mixed_types.csv", 5, {
            { .name = "id", .type = duckdb::LogicalTypeId::INTEGER },
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("nulls_and_empty.csv", 6, {
            { .name = "id", .type = duckdb::LogicalTypeId::INTEGER },
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("numeric_types.csv", 5, {
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("single_column.csv", 10, {
            { .name = "value", .type = duckdb::LogicalTypeId::SMALLINT }
        }),
        std::make_tuple<std::string, size_t, std::vector<column_def>>("special_chars.csv", 6, {
            { .name = "id", .type = duckdb::LogicalTypeId::INTEGER },
...
TEST_CASE("Test reading zstd-compressed CSV files", "[csv_processor]") {
    auto [filename, row_count, columns] = GENERATE(table<std::string, size_t, std::vector<column_def>>({
        std::make_tuple<std::string, size_t, std::vector<column_def>>("customers-100000.csv.zst", 100000, {
        ...
        std::make_tuple<std::string, size_t, std::vector<column_def>>("train_few_shot.csv.zst", 40000, {
            { .name = "text", .type = duckdb::LogicalTypeId::VARCHAR },
