  ...
  csv_processor::ProcessFile(
      con, props, logger, [&con, &test_file](const std::string &view_name) {
        ...
        const std::string compare_query =
            "WITH expected_data AS (FROM read_csv_auto($2, header = true)) "
        ...
        if (compare_prepared_stmt->HasError()) {
          FAIL("Failed to prepare comparison query: " +
               compare_prepared_stmt->GetError());
        }
        ...
        REQUIRE(compare_result->type ==
                duckdb::QueryResultType::MATERIALIZED_RESULT);
        REQUIRE(compare_result->Cast<duckdb::MaterializedQueryResult>()
                    .RowCount() == 0);
      });
}