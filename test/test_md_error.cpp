...
bool has_valid_utf8_end(const std::string_view str) {
	...
	const unsigned char last_byte = static_cast<unsigned char>(str[last_byte_pos]);
	...
	while (start_byte_pos > 0 && (static_cast<unsigned char>(str[start_byte_pos]) & 0b11000000) == 0b10000000) {
	...
	const unsigned char start_byte = static_cast<unsigned char>(str[start_byte_pos]);
...
TEST_CASE("Test truncate_for_grpc_header", "[md_error]") {
	...
	SECTION("Check that too long messages are truncated") {
		...
		const std::string truncated_message = md_error::truncate_for_grpc_header(long_message);
		REQUIRE(truncated_message.size() < long_message.size());
		REQUIRE(truncated_message.ends_with("...[truncated]"));
	}

	SECTION("Check that UTF-8 messages are truncated without breaking characters") {
		...
		const std::string_view message_content(truncated_message.data(), truncated_message.size() - suffix.size());
