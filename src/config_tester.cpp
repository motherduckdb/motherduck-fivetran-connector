...
TestResult run_database_type_test(duckdb::Connection& con) {
	...
	const auto prepared_stmt = con.Prepare("SELECT type FROM md_all_databases() WHERE is_attached AND alias = ?");
	...
	if (db_type.find("motherduck") == std::string::npos) {
		...
		return TestResult(false, "\"" + db_name + "\" is not a MotherDuck database, but has type \"" + db_type +
		                             "\". Please use a writable MotherDuck database instead.");
	}
...
TestResult run_write_permissions_test(duckdb::Connection& con) {
	...
	assert(duckling_id_res->RowCount() == 1 && duckling_id_res->ColumnCount() == 1);
	...
	if (duckling_id.find(".rs.") != std::string::npos) {
		return TestResult(false, "The provided authentication token is a read-scaling token. A token "
