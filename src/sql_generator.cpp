...
void MdSqlGenerator::add_partial_historical_values(duckdb::Connection &con, const table_def &table,
	...
	write_joined(sql, columns_regular,
							 [staging_table_name, absolute_table_name, unmodified_string](
									 const std::string quoted_col, std::ostringstream &out) {
									 out << "CASE WHEN " << staging_table_name
											 << "." << quoted_col << " = "
											 << KeywordHelper::WriteQuoted(unmodified_string, '\'')
											 << " THEN lar." << quoted_col
											 << " ELSE " << staging_table_name << "." << quoted_col
	...
	sql << " FROM " << staging_table_name << " LEFT JOIN latest_active_records lar ON ";

	write_joined(
			sql, columns_pk,
			[&](const std::string &quoted_col, std::ostringstream &out) {
					sql << "lar." << quoted_col << " = " << staging_table_name << "." << quoted_col;
			},
			" AND ");
	sql << ")";
...
void MdSqlGenerator::deactivate_historical_records(duckdb::Connection &con, const table_def &table,
	...
	// persist the data (it's only 2 columns) because Arrow streams can only be read once
...

	{
		...
		std::ostringstream sql;
		sql << "DELETE FROM " + absolute_table_name << " USING " << temp_earliest_table_name
				<< " WHERE ";
		sql << primary_key_join_condition;
		sql << " AND " << short_table_name << "._fivetran_start >= " << temp_earliest_table_name << "._fivetran_start";
	...
	// this will not be cleaned up manually just in case further update files need to refer to the latest values
	con.Query("CREATE TABLE IF NOT EXISTS latest_active_records AS (SELECT * FROM " + absolute_table_name + ") LIMIT 0");

	{
		// store latest active records before they get deactivated
		std::ostringstream sql;
		...
		sql <<  " ORDER BY " << short_table_name << "._fivetran_start DESC) as row_num FROM " << absolute_table_name;
		// inner join to earliest table to only select rows that are in this batch
		sql << " INNER JOIN " << temp_earliest_table_name << " ON " << primary_key_join_condition << ")\n";
		//sql << " WHERE _fivetran_active = TRUE";	// if a previous run failed, the records will have been turned to FALSE already

		sql << "INSERT INTO latest_active_records SELECT * EXCLUDE (row_num) FROM ranked_records WHERE row_num = 1";
		auto query = sql.str();
...

	{
		...
		// converting to TIMESTAMP with no timezone because otherwise ICU is required to do TIMESTAMPZ math. Need to test this well.
		sql << "_fivetran_end = (" << temp_earliest_table_name << "._fivetran_start::TIMESTAMP - (INTERVAL '1 millisecond'))"; // TBD subtract a millisecond
