...
public:
    Status ConfigurationForm(::grpc::ServerContext *context, const ::fivetran_sdk::ConfigurationFormRequest *request,
    ...
    fivetran_sdk::DataType get_fivetran_type(const LogicalTypeId &duckdb_type) const {
        switch (duckdb_type) {
            ...
            case LogicalTypeId::TIMESTAMP:
                return fivetran_sdk::UTC_DATETIME;   // TBD: this is pretty definitely wrong, and should naive time be returned for any reason?
    ...
    std::unique_ptr<Connection> get_connection(const google::protobuf::Map<std::string, std::string> &request_config, const std::string &db_name) {
        std::string token = find_property(request_config, "motherduck_token");

        std::unordered_map<std::string, std::string> props{{"motherduck_token", token}};
        DBConfig config(props, false);
        DuckDB db("md:" + db_name, &config);
        return std::make_unique<Connection>(db);
    }

    Status DescribeTable(::grpc::ServerContext *context, const ::fivetran_sdk::DescribeTableRequest *request,
                         ::fivetran_sdk::DescribeTableResponse *response) override {
        try {

            std::string db_name = find_property(request->configuration(), "motherduck_database");
            std::unique_ptr<Connection> con = get_connection(request->configuration(), db_name);

            if (!table_exists(*con, db_name, request->schema_name(), request->table_name())) {
                response->set_not_found(true);
                return ::grpc::Status(::grpc::StatusCode::OK, "");
            }

            auto duckdb_columns = describe_table(*con, db_name, request->schema_name(), request->table_name());
    ...
    LogicalTypeId get_duckdb_type(const fivetran_sdk::DataType& fivetranType) const {
        switch (fivetranType) {
    ...
    get_duckdb_columns(const google::protobuf::RepeatedPtrField<fivetran_sdk::Column> &fivetran_columns) const {
        std::vector<column_def> duckdb_columns;
        for (auto col: fivetran_columns) {
            // todo: if not decimal? (hasDecimal())
            duckdb_columns.push_back(
                    column_def{col.name(), get_duckdb_type(col.type()), col.primary_key(), col.decimal().precision(),
                               col.decimal().scale()});
        }
    ...
    std::string
    find_property(const google::protobuf::Map<std::string, std::string> &config, const std::string &property_name) const {
    ...
    Status CreateTable(::grpc::ServerContext *context, const ::fivetran_sdk::CreateTableRequest *request,
    ...
    Status AlterTable(::grpc::ServerContext *context, const ::fivetran_sdk::AlterTableRequest *request,
    ...
    Status Truncate(::grpc::ServerContext *context, const ::fivetran_sdk::TruncateRequest *request,
    ...
    std::vector<unsigned char> decrypt_file(const std::string &filename, const unsigned char *decryption_key) const {
        ...
        if (1 != EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, decryption_key, iv.data()))
            throw std::runtime_error("Could not decrypt file " + filename);
        int len;
        std::vector<unsigned char> plaintext(encrypted_data.size());
        if (1 != EVP_DecryptUpdate(ctx, plaintext.data(), &len, encrypted_data.data(), encrypted_data.size()))
    ...
    std::shared_ptr<arrow::Table> ReadCsv(const std::string &filename, const std::string &decryption_key) {


        std::vector<unsigned char> plaintext = decrypt_file(filename,
                                                            reinterpret_cast<const unsigned char *>(decryption_key.c_str()));

        auto buffer = std::make_shared<arrow::Buffer>(reinterpret_cast<const uint8_t *>(plaintext.data()),
                                                      plaintext.size());
        ...
        if (!maybe_codec.ok()) {
            throw std::runtime_error(
                    "Could not create codec from ZSTD compression type: " + maybe_codec.status().message());
        }
        auto codec = std::move(maybe_codec.ValueOrDie());
        auto maybe_compressed_input_stream = arrow::io::CompressedInputStream::Make(codec.get(), buffer_reader);
        ...
        auto compressed_input_stream = std::move(maybe_compressed_input_stream.ValueOrDie());

        auto read_options = arrow::csv::ReadOptions::Defaults();
        auto parse_options = arrow::csv::ParseOptions::Defaults();
        auto convert_options = arrow::csv::ConvertOptions::Defaults();

        auto maybe_table_reader = arrow::csv::TableReader::Make(arrow::io::default_io_context(),
                                                                std::move(compressed_input_stream), read_options,
                                                                parse_options, convert_options);
        if (!maybe_table_reader.ok()) {
            throw std::runtime_error("Could not create table reader: " + maybe_table_reader.status().message());
        }
        auto table_reader = std::move(maybe_table_reader.ValueOrDie());

        auto maybe_table = table_reader->Read();
        if (!maybe_table.ok()) {
            throw std::runtime_error("Could not read CSV" + maybe_table.status().message());
        }
    ...
    const std::string
    get_encryption_key(const std::string &filename, const google::protobuf::Map<std::string, std::string> &keys) {
    ...
    get_primary_keys(const google::protobuf::RepeatedPtrField<fivetran_sdk::Column> &columns) const {
    ...
    void process_file(Connection &con,
        ...
        if (!status.ok()) {
            throw std::runtime_error("Could not convert Arrow batch reader to an array stream: " + status.message());
        }
    ...
    Status WriteBatch(::grpc::ServerContext *context, const ::fivetran_sdk::WriteBatchRequest *request,
        ...
        try {
            ...
            // Use local memory by default to prevent Arrow-based VIEW from traveling up to the cloud
            ...
            for (auto &filename: request->replace_files()) {
                ...
                process_file(*con, filename, decryption_key,
                             [&con, &db_name, &request, &primary_keys, &cols](const std::string view_name) {
                                 upsert(*con, db_name, request->schema_name(), request->table().name(), view_name,
                                        primary_keys, cols);
                             });
            }
            for (auto &filename: request->update_files()) {
                ...
                process_file(*con, filename, decryption_key,
                             [&con, &db_name, &request, &primary_keys, &cols](const std::string view_name) {
                                 update_values(*con, db_name, request->schema_name(), request->table().name(), view_name,
                                               primary_keys, cols);
                             });
            }
            for (auto &filename: request->delete_files()) {
                auto decryption_key = get_encryption_key(filename, request->keys());
                process_file(*con, filename, decryption_key,
                             [&con, &db_name, &request, &primary_keys](const std::string view_name) {
                                 delete_rows(*con, db_name, request->schema_name(), request->table().name(), view_name,
                                             primary_keys);
                             });
            }
    ...
    Status Test(::grpc::ServerContext *context, const ::fivetran_sdk::TestRequest *request,
...
int main(int argc, char **argv) {
    ...
    for (auto i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--port") == 0) {
            if (i + 1 >= argc) {
                throw std::runtime_error(
                        "Please provide a port number.\nUsage: motherduck_destination [--port <PORT>]");
            }
            port = argv[i + 1];
        }
