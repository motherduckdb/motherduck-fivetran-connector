...
MemoryBackedFile MemoryBackedFile::Create(const size_t file_size) {
	...
	const int fd = memfd_create("fivetran_decrypted.csv", MFD_CLOEXEC | MFD_ALLOW_SEALING);
	if (fd == -1) {
		throw std::system_error(errno, std::generic_category(), "Failed to create memfd");
	}
#else
	...
	if (mkdir(tmp_dir.c_str(), 0700) == -1 && errno != EEXIST) {
		throw std::system_error(errno, std::generic_category(), "Failed to create temp directory " + tmp_dir);
	}
	...
	if (fd == -1) {
		throw std::system_error(errno, std::generic_category(), "Failed to create temp memfile " + tmp_path);
	}
	...
	if (unlink(tmp_path.c_str()) == -1) {
		close(fd);
		throw std::system_error(errno, std::generic_category(), "Failed to unlink temp memfile " + tmp_path);
	}
#endif
	...
	if (ftruncate(fd, static_cast<off_t>(file_size)) == -1) {
		...
		throw std::system_error(errno, std::generic_category(),
		                        "Failed to truncate temp memfile with fd=" + std::to_string(fd));
	}
